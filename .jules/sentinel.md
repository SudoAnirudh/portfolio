...
## 2026-07-27 - Enforce Client-Side MaxLength to Match Server Limits
**Vulnerability:** Client-side form inputs lack explicit length limits, allowing rudimentary application-layer DoS via oversized payloads that are only rejected by the server.
**Learning:** Client-side form inputs must enforce `maxLength` attributes to explicitly align with their server-side validation counterparts to prevent unnecessary server load and poor UX.
**Prevention:** Always add `maxLength` attributes to client-side inputs that correspond to server-side length validations.

...
## 2024-07-26 - [Client-Side DoS Prevention via MaxLength]
**Vulnerability:** Client-side form inputs lacked length restrictions, allowing the application to be overloaded with massive payloads prior to server validation, posing a rudimentary DoS risk.
**Learning:** Enforcing constraints solely on the server is insufficient; client-side constraints should align with server-side counterparts (e.g., maxLength for inputs) to prevent application-layer resource exhaustion at the browser level and provide early feedback.
**Prevention:** Always enforce client-side maxLength attributes on form inputs that explicitly align with their server-side validation counterparts (e.g., Name: 100, Email: 255, Message: 5000).

...
## 2024-07-07 - Add Permissions-Policy Header
**Vulnerability:** Missing `Permissions-Policy` header.
**Learning:** Adding modern security headers in `next.config.ts` like `Permissions-Policy` restrict access to browser features (camera, microphone, geolocation, browsing-topics), which provides defense-in-depth against third-party iframes and potential XSS execution contexts.
**Prevention:** Always ensure an exhaustive array of security headers (including `Permissions-Policy`) is present in the Next.js global `headers()` config block.

...
## 2025-02-15 - Media Player and Retro UI Controls Accessibility
**Learning:** Custom UI components mimicking retro interfaces (like `RetroMusicPlayer.tsx` and custom windows) often lack native semantic elements. Icon-only buttons (like "minimize", "close", "play", "pause") were missing `aria-label`s, causing screen readers to read the raw Google Material Symbol font text or announce them as unlabelled buttons. Form inputs inside these custom widgets (like the volume slider) were also missing `htmlFor`/`id` associations.
**Action:** When creating or modifying custom widget controls, ensure all icon-only interactive elements have descriptive `aria-label`s and that any text-based icon fonts (like `material-symbols-outlined`) have `aria-hidden="true"`. Additionally, verify that all custom form elements (sliders, inputs) have their `<label>` properly linked via `htmlFor` and `id` to provide proper context to assistive technologies.

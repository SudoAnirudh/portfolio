...
## 2024-10-24 - [Denial of Service via Oversized Payloads on Subject Field]
**Vulnerability:** The server action `submitContactForm` checked for the presence of `name`, `email`, and `message` and had length validation for them, but lacked input length constraints for the optional `subject` field. This left the endpoint vulnerable to Denial of Service (DoS) attacks where a malicious actor could send massive string payloads in the subject field, leading to excessive memory consumption during text manipulation and rendering inside the Resend API payload.
**Learning:** Checking for truthiness is not sufficient for input validation on server-side endpoints, and even optional fields need length restrictions to prevent resource exhaustion, especially when the data is directly forwarded to external APIs or interpolated into HTML templates.
**Prevention:** Enforce strict type checking, regex validation (for emails), and reasonable length constraints (e.g., Subject: max 200) for all server action arguments before processing the payload.
